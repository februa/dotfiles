...
export class Config extends BaseConfig {
  override async config(args: {
    ...
    // 修正: stdpath("config") のパスを使う
    const configDir = await args.denops.call("stdpath", "config") as string;
    const tomlPath = configDir + "/dpp/plugins.toml";
    
    // TOML から読み込み
    const tomlAction = await args.dpp.extAction(
      args.denops,
      context,
      options,
      "toml",
    ...
    return {
      plugins: tomlAction.plugins ?? [],
      stateLines: tomlAction.stateLines ?? [],
    };
  }
}
